...
import { User, Calendar, CurrencyDollar, House, Clock, FirstAid, Receipt, Camera } from '@phosphor-icons/react';
    ...
    <div className="space-y-6">
      ...
      {appointments.length === 0 ? (
        <div className="bg-white p-8 rounded-xl shadow text-center">
          ...
          <Link href="/appointment" className="mt-4 inline-block px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
            Book an Appointment
          </Link>
        </div>
      ) : (
  ...
  const AccountView = () => {
    ...
    const uploadProfileImage = async (file) => {
      ...
      try {
        ...
        if (accountData.photoUrl && accountData.photoUrl.startsWith('https://')) {
          const oldImageRef = ref(storage, accountData.photoUrl);
          await deleteObject(oldImageRef).catch(() => {});
        }

        // Upload new image
        const storageRef = ref(storage, `profile_images/${user.uid}/${Date.now()}`);
